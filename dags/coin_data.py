...
def filter_coin_json(data: dict) -> dict:
    return {
        "id": data.get("id"),
...
@dag(
    ...
    schedule="0 1 * * *", # at 1:00 UTC to avoid overlapping with coin_price DAG (due to the rate limits)
    catchup=False,
    tags=['crypto', 'daily']
)
def coin_data():
    ...
    def get_coin_data(url: str, coin_id: str) -> str:
        import requests
        
        url = f"{url}{coin_id}?localization=false&tickers=false&market_data=false&community_data=false&developer_data=false"
        api = BaseHook.get_connection('coingecko_api')
        response = requests.get(url, headers=json.loads(api.extra))
        print(url)

        # Filter the JSON response to keep only the necessary fields
        filtered = filter_coin_json(response.json())
    ...
    @task
    def store_coin_data(coin: str, data_interval_start=None):
        
        upload_json_to_s3(
                    s3_hook=S3Hook(aws_conn_id="AwsS3Bucket"),
                    bucket=BUCKET_NAME,
                    key=f"bronze/coin_data/{data_interval_start.strftime("%Y-%m-%d")}/coin_data_{json.loads(coin)['id']}.json",
                    data=coin
                )

    transform_coin_data = SparkSubmitOperator(
        task_id='transform_coin_data',
        application='/opt/airflow/dags/notebooks/scripts/dim_coin_data.py',
    ...
    # Get coin data dynamically
    coin_data_results = get_coin_data.expand(
        url=is_api_available(),  # XCom value from sensor
        coin_id=coin_ids         # list of coin_ids
    )

    # Store each coin's data in S3
    store_coin_data.expand(
        coin=coin_data_results
    ) >> transform_coin_data
